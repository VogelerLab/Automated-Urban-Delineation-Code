...
This python file is for the final year (t2)
"""
...
#We default to referring to the final year in the study period as t2
year = "t2"

#Enter code for project as a string
#Current projection: Africa Albers Equal Area
...
fg_ue_t2 = processing.run('native:fixgeometries',
               {'INPUT':raw_urbanextent,
                'OUTPUT':'memory:'})

fg_ue_t2 = fg_ue_t2['OUTPUT']

#Reproject population data
pg_t2_fp = os.path.join(dir_01_outputs, "pop_grid_" + country_abbrv + "_" +  year + ".tif")
...
#urban_clusters_t2 = os.path.join(dir_01_outputs, "ue_clusters_" + country_abbrv + "_" +  year + ".shp")

urban_clusters_t2 = processing.run("native:clip", 
               {'INPUT':fg_ue_t2,
                'OVERLAY':country_boundary,
                'OUTPUT':'memory:'})
                
urban_clusters_t2 = urban_clusters_t2['OUTPUT']

#Zonal statistics to determine population within each urban cluster

ue_with_popdata_t2 = processing.run("native:zonalstatisticsfb", 
               {'INPUT':urban_clusters_t2,
                'INPUT_RASTER':pg_t2_fp,
                'RASTER_BAND':1,
                'COLUMN_PREFIX':'_',
                'STATISTICS':[0,1,2],
                'OUTPUT':'memory:'}) 

ue_with_popdata_t2 = ue_with_popdata_t2['OUTPUT']

# Calculate area field for polygons

ue_area_m = processing.run("native:fieldcalculator",
               {'INPUT':ue_with_popdata_t2,
                'FIELD_NAME':'area(m²)',
                'FIELD_TYPE':0,
                'FIELD_LENGTH':0,
                'FIELD_PRECISION':0,
...
#ue_pop_t2_greater_than_threshold = os.path.join(dir_01_outputs, 'UE_Pop_ET_t2_Greater_Than_Threshold.shp') 
...
#town_cores = os.path.join(dir_01_outputs, 't2_Town_Cores.shp')
